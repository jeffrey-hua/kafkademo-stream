...
public class PurchaseAnalysisTest2 {
...
//        KTable<NameCategory, QuantityPrice> kTable = orderStream
...
//                .reduce((QuantityPrice v1, QuantityPrice v2) -> QuantityPrice.fromQuantityPrice(v1.getQuantity() + v2.getQuantity(), v1.getPrice()), "gender-amount-state-store");
//
//        kTable
//                .toStream()
//                .map((NameCategory nameCategory, QuantityPrice quantityPrice) -> new KeyValue<String, String>(nameCategory.toString(), quantityPrice.toString()))
//                .to("gender-amount");

//        KStream kStream = kTable.toStream()
//                .map((NameCategory nameCategory, QuantityPrice quantityPrice) -> KeyValue.<String, String>pair(nameCategory.toString(),quantityPrice.toString())).groupByKey().aggregate(
//                        () -> "",
//                        (aggKey, value, quantityPrice) -> "1" ,
//                        TimeWindows.of(2000).advanceBy(1000),
//                        Serdes.String(),
//                        "Counts")
//                .toStream()
//                .map((Windowed<String> window, String value) -> {
//                    return new KeyValue<String, String>(String.format("start=%d, end=%d, key=%s\n",window.window().start(), window.window().end(), window.key()),value);
//                });
        orderStream
                .leftJoin(userTable, (Order order, User user) -> OrderUser.fromOrderUser(order, user), Serdes.String(), SerdesFactory.serdFrom(Order.class))
                .filter((String userName, OrderUser orderUser) -> orderUser.userAddress != null)
        ...
        /*KStream kStream = kTable
                .toStream()
                .map((NameCategory nameCategory, QuantityPrice quantityPrice) -> KeyValue.<NameCategory, QuantityPrice>pair(nameCategory,quantityPrice)).groupByKey().aggregate(
                () -> new QuantityPrice(),
                (aggKey, value, quantityPrice) -> new QuantityPrice(1,1d) ,
                TimeWindows.of(5000).advanceBy(5000),
                SerdesFactory.serdFrom(QuantityPrice.class),
                "Counts")
                .toStream()
                .map((Windowed<NameCategory> window, QuantityPrice value) -> {
                    return new KeyValue<String, String>(String.format("start=%d, end=%d, category=%s, itemname=%s, username=%s\n",value, window.window().start(), window.window().end(), window.key().getCategory(),
                            window.key().getItemName(),window.key().getUserNname()),
                            String.format("quantity=%s, price=%s, total=%d \n",value.getQuantity(),value.getPrice(),value.getTotal()));
                });
        kStream.to(Serdes.String(), Serdes.String(), "gender-amount");
        KafkaStreams kafkaStreams = new KafkaStreams(streamBuilder, props);
        kafkaStreams.cleanUp();
        kafkaStreams.start();
        ...
        /*KTable<NameCategory, QuantityPrice> kTable = orderStream
                ...
                .map((String item, OrderUserItem orderUserItem) -> KeyValue.<NameCategory, QuantityPrice>pair(new NameCategory(orderUserItem.userName,orderUserItem.itemName,orderUserItem.itemCategory), QuantityPrice.fromQuantityPrice(orderUserItem.quantity, orderUserItem.itemPrice)))
                .groupByKey(SerdesFactory.serdFrom(NameCategory.class), SerdesFactory.serdFrom(QuantityPrice.class))
                .reduce((QuantityPrice v1, QuantityPrice v2) ->
                        new QuantityPrice(v1.quantity + v2.quantity, v1.price + v2.price), "gender-amount-state-store");
        kTable
                .toStream()
                .map((NameCategory nameCategory, QuantityPrice quantityPrice) -> new KeyValue<String, String>(nameCategory.toString(), quantityPrice.toString()))
                .to("gender-amount");
